...
class EncoderLayer_AugVeh(nn.Module):
    ...
    def forward(self, 
        ...
        if self.encoder_version == 7:
            proc_trans_time = torch.cat([proc_time.transpose(1, 2), trans_time], dim=-1)                              
            ope_ma_emb = torch.cat([ope_emb, ma_emb], dim=1)                              
            ma_emb_out = self.ma_encoding_block(ma_emb, ope_ma_emb, proc_trans_time)                  
            veh_emb_out = self.veh_encoding_block(veh_emb, ma_emb, MVpair_trans)                   
            ope_emb_out = self.job_encoding_block(ope_emb, ma_emb, veh_emb, proc_time, offload_trans_time)                   
            ...
            ope_ma_veh_emb = torch.cat([ope_emb, ma_emb, veh_emb], dim=1)                                     
            ma_emb_out = self.ma_encoding_block(ma_emb, ope_ma_veh_emb, proc_OnOffTrans_time)                  
            veh_emb_out = self.veh_encoding_block(veh_emb, ma_emb, MVpair_trans)                   
            ope_emb_out = self.job_encoding_block(ope_emb, ma_emb, proc_time)                   
        elif self.encoder_version == 12:
                                             
            proc_OnOffTrans_time = torch.cat([proc_time.transpose(1, 2), trans_time, MVpair_trans.transpose(1, 2)], dim=-1)   
            ope_ma_veh_emb = torch.cat([ope_emb, ma_emb, veh_emb], dim=1)                                     
            tmp_ma_emb_out = self.ma_encoding_block(ma_emb, ope_ma_veh_emb, proc_OnOffTrans_time)                  
            tmp_veh_emb_out = self.veh_encoding_block(veh_emb, ma_emb, MVpair_trans)                   
            tmp_ope_emb_out = self.job_encoding_block(ope_emb, ma_emb, proc_time)                   
            
            total_emb = torch.cat([tmp_ope_emb_out, tmp_ma_emb_out, tmp_veh_emb_out], dim=1)                        
            total_emb_out = self.total_encoding_block(total_emb, total_emb)                     
...
class EncodingBlock_Job(EncodingBlock_Base):
    ...
    def forward(self, ope_emb, ma_emb, veh_emb, proc_time_mat, trans_time_mat, ope_adj_mat=None):
        ...
        ma_veh_emb = torch.cat([ma_emb, veh_emb], dim=1)                            
        proc_trans_time_mat = torch.cat([proc_time_mat, trans_time_mat], dim=-1)                                 

        q = reshape_by_heads(self.Wq(ope_emb), head_num=head_num)                            

        k = reshape_by_heads(self.Wk(ma_veh_emb), head_num=head_num)                         
        v = reshape_by_heads(self.Wv(ma_veh_emb), head_num=head_num)

        out_concat = self.mixed_score_MHA(q, k, v, proc_trans_time_mat)                          
        multi_head_out = self.multi_head_combine(out_concat)
