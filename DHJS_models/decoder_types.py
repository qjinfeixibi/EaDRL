...
class TFJSP_Decoder_DHJS_V2(TFJSP_Decoder_DHJS_Base):
    ...
    def _select_node(self, global_merge, ninf_mask, 
        ...
        Output:
            ...
            node_prob: [B, 1]
        '''
        score = torch.matmul(global_merge, single_head_key)                              
        ...
        if softmax:
            while True:                                                                      
                select_node = all_node_prob.reshape(batch_size, -1).multinomial(1).squeeze(dim=1).reshape(batch_size, 1)         
    ...
    def _get_global_atten(self, context, embed_opes, embed_mas, embed_vehs):
        ...
        '''
        global_ope = self.atten_to_job(context.squeeze(1), embed_opes)               
        global_ma = self.atten_to_ma(context.squeeze(1), embed_mas)
        global_veh = self.atten_to_veh(context.squeeze(1), embed_vehs)
        
        global_node = torch.cat([context, global_ope, global_ma, global_veh], dim=-1)                 
        global_merge = self.proj_glimp_ope(global_node)            
    ...
    def forward(self, embed_nodes, embed_context, prev_emb, state, mask, mask_ope_ma, mask_veh,
        ...
        mask_job = torch.where(mask.sum(dim=2) > 0, True, False)                 
        ...
        ninf_mask_ma_on_job = ninf_mask.gather(1, select_job[:, :, None].expand(-1, -1, ninf_mask.size(2)))                
...
class TFJSP_Decoder_DHJS_V3(TFJSP_Decoder_DHJS_Base):
    ...
    def forward(self,
        ...
        select_veh = select_veh_dict['veh_id'].unsqueeze(1).long()         
        embed_select_veh = embed_vehs.gather(1, select_veh[:, :, None].expand(-1, -1, embed_vehs.size(-1)))             
