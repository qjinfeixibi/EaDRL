...
class TFJSPTrainer_DHJS():
    def __init__(self,
        ...
        self.opes_per_job_min = int(self.num_mas * 0.9)
    ...
    def _train_one_batch(self, batch_size, env, dataset=None, loader=None):
        self.model.train()
        state = env.reset()
        ...
        dones = env.done_batch
        logits_list = []
        max_steps = self.train_paras.get("max_steps", None)
        step_count = 0
        ...
        while not done:
            ...
            state, reward, dones = env.step(action, step_reward="version2")
            log_probs.append(log_p.squeeze(-1))           
            rewards.append(reward)                        
            values.append(value.squeeze(-1))              
            logits_list.append(logits)
            dones_hist.append(dones.float())              
            step_count += 1
            done = dones.all()
        ...
        with torch.no_grad():
            if env.done or dones.all():
                ...
                _, _, next_value = self.model.act(state, return_state_vec=True)
        ...
        dones_t  = torch.stack(dones_hist) if len(dones_hist)>0 else torch.zeros_like(rewards)         
        ...
        gamma_adv = self.train_paras.get("gamma", 1.0)
        lam = self.train_paras.get("gae_lambda", 0.95)
        gamma_ret = self.train_paras.get("gamma", 1.0)
        advantages, returns = compute_gae(
            rewards, values, dones=dones_t, gamma_adv=gamma_adv, lam=lam, gamma_ret=gamma_ret
        )                
        ...
        actor_loss  = -(adv_norm.detach() * log_probs).mean()
        critic_loss = F.mse_loss(values[:-1], value_target)
        vf_coeff = self.train_paras.get("vf_coeff", 0.5)
        total_loss  = actor_loss + vf_coeff * critic_loss - entropy_coeff * entropy_bonus
        self.optimizer.zero_grad()
        total_loss.backward()
        max_grad_norm = self.train_paras.get("max_grad_norm", 0)
        if max_grad_norm:
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        self.result_log.data = checkpoint['result_log']
        self.start_epoch = checkpoint['epoch'] + 1
        self.logger.info(f"Loaded checkpoint from {checkpoint_path}, resuming from epoch {self.start_epoch}")

def compute_gae(rewards, values, dones=None, gamma_adv=0.95, lam=1.0, gamma_ret=1.0):
    T = rewards.size(0)
    device = rewards.device
    dtype = rewards.dtype

    if dones is None:
        not_done = torch.ones_like(rewards, dtype=dtype, device=device)
    else:
    ...
    for t in reversed(range(T)):
        delta = rewards[t] + gamma_adv * not_done[t] * values[t + 1] - values[t]
        gae = delta + gamma_adv * lam * not_done[t] * gae
        advantages[t] = gae

        next_return = rewards[t] + gamma_ret * not_done[t] * next_return
