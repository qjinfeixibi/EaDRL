...
class EncoderLayer_intent(nn.Module):
    ...
    def forward(self, 
        ...
        A_proc_time = self.min_ProcTime_elements(A_proc_time)                       
        ...
        A_nonprocing_ma_mask = (tmp_row_mask == True) & (tmp_col_mask == True)                     
        A_procing_ma = (~A_nonprocing_ma_mask).float()
        
                                           
        A_offloadTrans_time_norm = self.inverse_normalize_matrix(MVpair_trans)                      
                                                                
        trans_time[~A_nonprocing_ma_mask] = self.transtime_btw_ma_max+1
        A_onloadTrans_time_norm = self.inverse_normalize_matrix(trans_time)                     
        ...
        if self.encoder_version == 9:
            ...
            ope_emb_padd = self._zero_pad_matrix(tmp_ope_emb_out, num_nodes-num_opes)                           
            ...
            emb_stack = torch.stack([ope_emb_padd, ma1_emb_padd, ma2_emb_padd, veh_emb_padd])                              
            emb_stack_trans = emb_stack.permute(1, 2, 0, 3).reshape(
                batch_size*num_nodes, self.num_intent, self.embedding_dim
            )                                   
            self.rnn.flatten_parameters()
            node_emb_out, _ = self.rnn(emb_stack_trans)                                 
            node_emb_out = node_emb_out.sum(dim=1)                           
            node_emb_out = self.norm(node_emb_out).reshape(batch_size, num_nodes, -1)                        
