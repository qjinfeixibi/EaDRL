...
def subgraph_meta_learner(
    ...
    ope_ope_adj = main_graph.ope_adj_batch.float()                        
    ope_ma_adj = torch.where(main_graph.ope_ma_adj_batch>0, 1., 0.)                     
    ope_veh_adj = torch.where(main_graph.ope_veh_adj_batch>0, 1., 0.)                                
    adj_mat_batch = torch.cat([ope_ope_adj, ope_ma_adj], dim=-1)                              
...
def get_kcore_case_list_from_adj_mat(
    ...
    k_core_graph_list = k_core_graph_list[::-1]                                                    
...
def get_feasible_subgraphs(
    ...
    for graph_idx in range(num_kcore_graph):
        tmp_ope_ma_adj = kcore_graph_list[graph_idx][:num_opes, num_opes:num_opes+num_mas]                   
        ...
        elig_ope_ma_adj = tmp_ope_ma_adj.gather(0, elig_node[:, None].expand(-1, num_mas))                        
...
def get_kcore_graph(num_opes, num_mas, num_vehs, adj_mat, spmat=True, draw_fig=False):
    ...
    for i in range(0, max_core_num+1):
        ...
        ope_conn_att = connected_nodes(A[:num_opes, :num_opes])                                   
