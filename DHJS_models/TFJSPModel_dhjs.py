...
class TFJSPModel_DHJS(nn.Module):
    ...
    def init(self, state, dataset=None, loader=None):
        ...
        if self.encoder_version == 3:
            ...
            trans_time = deepcopy(state.trans_times_batch)                      
            _, offload_trans_time, _, _ =\
                generate_trans_mat(trans_time, state, job_embedding=False)                                                
            
            adj_mat = torch.cat([oper_adj_batch, proc_time, offload_trans_time], dim=-1)                                          
            ...
            OO_adj_mat = state.ope_adj_batch.float()                         
            OM_adj_mat = torch.where(state.proc_times_batch>0, 1., 0.)                      
            OV_adj_mat = torch.ones(size=(self.batch_size, self.num_opes, self.num_vehs))
            adj_mat = torch.cat([OO_adj_mat, OM_adj_mat, OV_adj_mat], dim=-1)                         
