...
class TFJSP_Decoder_DHJS_Base(nn.Module):
    ...
    def set_ope_kv(self, encoded_opes):
        ...
        self.k_ope = reshape_by_heads(self.Wk_ope(encoded_opes), head_num=head_num)                             
        self.v_ope = reshape_by_heads(self.Wv_ope(encoded_opes), head_num=head_num)                             
    ...
    def set_ma_kv(self, encoded_mas):
        ...
        '''
        head_num = self.model_params['head_num']
        self.k_ma = reshape_by_heads(self.Wk_ma(encoded_mas), head_num=head_num)                            
        self.v_ma = reshape_by_heads(self.Wv_ma(encoded_mas), head_num=head_num)                            
    ...
    def set_veh_kv(self, encoded_vehs):
        ...
        '''
        head_num = self.model_params['head_num']
        self.k_veh = reshape_by_heads(self.Wk_veh(encoded_vehs), head_num=head_num)                             
        self.v_veh = reshape_by_heads(self.Wv_veh(encoded_vehs), head_num=head_num)                             
        self.single_head_key_veh = encoded_vehs.transpose(1, 2)                 
    ...
    def forward(self, embed_nodes, embed_context, prev_emb, state, mask, mask_ope_ma, mask_veh,
        ...
        ninf_mask_veh = torch.where(mask_veh == True, 0., -math.inf)[:, None, :]                    
        
        if job_embedding:
            num_opes_jobs = num_jobs
            
            ninf_mask_ope_ma = torch.where(mask == True, 0., -math.inf)                      
            mask_ope = torch.where(mask.sum(dim=2) > 0, True, False)                 
        else:
            num_opes_jobs = num_opes
            
            ninf_mask_ope_ma = torch.where(mask_ope_ma == True, 0., -math.inf)                      
            mask_ope = torch.where(mask_ope_ma.sum(dim=2) > 0, True, False)                 
        ninf_mask_ope_job = torch.where(mask_ope == True, 0., -math.inf)[:, None, :]                            
        ninf_mask_nodes = torch.cat([ninf_mask_ope_job, ninf_mask_ma, ninf_mask_veh],dim=-1)                     
        
        
                                 
        embed_graph = torch.mean(embed_nodes, dim=1, keepdim=True)                 
        ...
        tmp_context = self.proj_context(torch.cat([embed_graph, prev_emb], dim=-1))                 
                               
        q_nodes = reshape_by_heads(self.Wq_nodes(tmp_context), head_num=head_num)                       
        
        out_nodes = self._multi_head_attention_for_decoder(q_nodes, self.k_nodes, self.v_nodes,
                                                            rank3_ninf_mask=ninf_mask_nodes)                            
        context = self.multi_head_combine_nodes(out_nodes)             
        ...
        ninf_mask_ma_on_ope = ninf_mask_ope_ma.gather(1, select_job_ope[:, :, None].expand(-1, -1, ninf_mask_ope_ma.size(2)))                
        ...
        embed_select_veh = embed_vehs.gather(1, select_veh[:, :, None].expand(-1, -1, embed_vehs.size(-1)))             

                             
        action = torch.cat([select_ope, select_ma, select_job, select_veh], dim=1).transpose(1, 0)          
        prob = ope_prob * ma_prob * veh_prob
        
        select_embed = torch.cat([embed_select_ope, embed_select_ma, embed_select_veh], dim=-1)                  
    ...
    def _select_node(self, q, k, v, ninf_mask, 
        ...
        mh_atten_out = multi_head_combine(out_concat)             
        ...
        all_node_prob = F.softmax(score_masked, dim=2).squeeze(1)                          
        logits = score_masked.squeeze(1)         
                        
        if softmax:
            dist = Categorical(all_node_prob)                                                             
            select_node = dist.sample().unsqueeze(-1)                                      
            node_prob = all_node_prob.gather(1, select_node)                             
            node_prob[finish_batch] = 1                                                                                  
            ...
            select_node = all_node_prob.argmax(dim=1).unsqueeze(1)            
            node_prob = torch.zeros(size=(batch_size, 1))                      
        
        
        
        
        return select_node, node_prob, mh_atten_out, logits
    ...
    def from_ope_to_job(self, select_ope, ope_step_batch, num_jobs, num_opes):
        ...
        select_job = torch.where(ope_step_batch == select_ope[:, None].expand(-1, num_jobs))[1]       
