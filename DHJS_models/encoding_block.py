...
class EncodingBlock_JobAdj(EncodingBlock_Base):
    ...
    def forward(self, ope_emb, ma_emb, veh_emb, proc_time_mat, trans_time_mat, ope_adj_mat):
        ...
        opeAdj_proc_trans_time_mat = torch.cat([ope_adj_mat, proc_time_mat, trans_time_mat], dim=-1)                                          

        q = reshape_by_heads(self.Wq(ope_emb), head_num=head_num)                            

        k = reshape_by_heads(self.Wk(ope_ma_veh_emb), head_num=head_num)                         
        v = reshape_by_heads(self.Wv(ope_ma_veh_emb), head_num=head_num)

        out_concat = self.mixed_score_MHA(q, k, v, opeAdj_proc_trans_time_mat)                          
        multi_head_out = self.multi_head_combine(out_concat)
