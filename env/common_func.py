...
def generate_trans_mat(
    ...
    allo_ma_batch = state.allo_ma_batch              
    elig_vehs = ~state.mask_veh_procing_batch                
    
    if job_embedding:
        ma_loc = prev_ope_locs_batch                 
    else:
        ma_loc = allo_ma_batch                       
    
    MVpair_trans_batch = trans_time_batch.gather(1, veh_loc_batch[:, :, None].expand(-1, -1, trans_time_batch.size(2)))                        
    empty_trans_batch = MVpair_trans_batch.gather(2, ma_loc[:, None, :].expand(-1, MVpair_trans_batch.size(1), -1))                         
    travel_trans_batch = trans_time_batch.gather(1, ma_loc[:, :, None].expand(-1, -1, trans_time_batch.size(2)))                       
    trans_mat = empty_trans_batch[:, :, :, None].expand(-1, -1, -1, num_mas) + travel_trans_batch[:, None, :, :].expand(-1, num_vehs, -1, -1)                               
...
def select_vehicle_v2(state, select_ma, select_ope, job_embedding=True):
    ...
    sel_offload_trans = offload_trans[batch_idxes, select_ope.squeeze(1), :]              
    output_onload_time = onload_trans[batch_idxes, select_ope.squeeze(1), select_ma.squeeze(1)]         
    output_offload_time, veh_id = sel_offload_trans.min(dim=1)        
    results = {
        'veh_id': torch.zeros(size=(batch_size,)),
        'trans_time': torch.zeros(size=(batch_size,)),
    }
...
def get_normalized(raw_opes, raw_mas, raw_vehs, proc_time, trans_time, flag_sample=False, flag_train=False):
    ...
    if not flag_sample and not flag_train:
        ...
        proc_time_norm = feature_normalize(proc_time)                                                
        trans_time_norm = feature_normalize(trans_time)                                              
...
def norm_disc_rewards(memory_rewards, memory_is_terminals, gamma, device):
    ...
    discounted_reward_batch = torch.stack(discounted_reward_batch, dim=0)         
