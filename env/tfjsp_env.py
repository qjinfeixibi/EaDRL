...
class TFJSPEnv(gym.Env):
    ...
    def _sample_poisson_release_times(self, n_new_jobs, max_job_comp_times):
        """
        Sample release times from a homogeneous Poisson arrival process.

        We model the first n_new_jobs arrivals by sampling i.i.d. exponential
        inter-arrival times with a constant rate within each instance, and then
        cumulatively summing them to obtain release times.
        """
        if n_new_jobs <= 0:
            return torch.zeros((self.batch_size, 0), dtype=torch.long, device=self.device)

        avg_num_ops = self.nums_ope_batch.float().mean(dim=1)       
        valid_proc_mask = self.proc_times_batch > 0
        proc_sum = self.proc_times_batch.float().sum(dim=(1, 2))
        proc_cnt = valid_proc_mask.sum(dim=(1, 2)).clamp(min=1).float()
        avg_proc_time = proc_sum / proc_cnt       

        mean_interarrival = (avg_num_ops * avg_proc_time / max(self.num_mas, 1)).clamp(min=1.0)
        rate = torch.reciprocal(mean_interarrival)

        release_rows = []
        for b in range(self.batch_size):
            inter_arrivals = torch.distributions.Exponential(rate[b]).sample((n_new_jobs,))
            release_times = torch.ceil(torch.cumsum(inter_arrivals, dim=0)).long().clamp(min=1)
            release_rows.append(release_times)

        return torch.stack(release_rows, dim=0).to(self.device)
    ...
    def __init__(self, case, env_paras, data_source='case', new_job_dict=None):
        ...
        self.ope_veh_adj_batch = torch.ones(size=(self.batch_size, self.num_opes, self.num_vehs)).long()                      
        self.ope_pre_adj_batch = torch.stack(tensors[2], dim=0)                                        
        self.ope_sub_adj_batch = torch.stack(tensors[3], dim=0)                                        
        self.cal_cumul_adj_batch = torch.stack(tensors[4], dim=0).float()                              
        self.nums_ope_batch = torch.stack(tensors[5], dim=0)                                     
        self.num_ope_biases_batch = torch.stack(tensors[6], dim=0).long()                     
        self.end_ope_biases_batch = torch.stack(tensors[7], dim=0).long()                     
        self.opes_appertain_batch = torch.stack(tensors[8], dim=0).long()                     
        self.trans_times_batch = torch.stack(tensors[9], dim=0)                                      
        self.ma_veh_adj_batch = torch.stack(tensors[10], dim=0).long()                                
        ...
        self.time = torch.zeros(size=(self.batch_size,))                                   
        self.N = torch.zeros(size=(self.batch_size,)).int()                               
        ...
        feat_opes_batch[:, PROC_TIME, :] = torch.sum(self.proc_times_batch, dim=2).div(feat_opes_batch[:, 1, :] + 1e-9)                                                                          
        feat_opes_batch[:, N_UNSCHED_OPE, :] = convert_feat_job_2_ope(self.nums_ope_batch, self.opes_appertain_batch)  
        feat_opes_batch[:, ESTI_START, :] = torch.bmm(feat_opes_batch[:, PROC_TIME, :].unsqueeze(1), self.cal_cumul_adj_batch).squeeze()                                                                                                
        end_time_batch = (feat_opes_batch[:, ESTI_START, :] + feat_opes_batch[:, PROC_TIME, :]).gather(1, self.end_ope_biases_batch)                                                             
        ...
        self.min_esti_start = torch.bmm(self.min_proc_times_batch.unsqueeze(1), self.cal_cumul_adj_batch).squeeze()                
    ...
    def step(self, action, step_reward=None):
        ...
        empty_trans_time = self.trans_times_batch[self.batch_idxes, self.veh_loc_batch[self.batch_idxes, veh], prev_ope_loc]          
        ...
        for i in range(self.batch_idxes.size(0)):
            self.feat_opes_batch[self.batch_idxes[i], N_UNSCHED_OPE, start_ope[i]:end_ope[i]+1] -= 1                                                   
        ...
        end_times_batch = (self.feat_opes_batch[self.batch_idxes, ESTI_START, :] + mean_trans_time + self.feat_opes_batch[self.batch_idxes, PROC_TIME, :]).gather(1, self.end_ope_biases_batch[self.batch_idxes, :])              
        ...
        self.feat_opes_batch[self.batch_idxes, N_NEIGH_MA, :] = torch.count_nonzero(self.dyn_ope_ma_adj_batch[self.batch_idxes, :, :], dim=2).float()                 
        self.feat_opes_batch[self.batch_idxes, N_NEIGH_VEH, :] = torch.count_nonzero(self.dyn_ope_veh_adj_batch[self.batch_idxes, :, :], dim=2).float()                 
        
                                                 
        self.sched_opes_batch[self.batch_idxes, ope, :2] = torch.stack((torch.ones(self.batch_idxes.size(0)), ma), dim=1)                               
        self.sched_opes_batch[self.batch_idxes, :, 2] = self.feat_opes_batch[self.batch_idxes, ESTI_START, :]                
        self.sched_opes_batch[self.batch_idxes, :, 3] = self.feat_opes_batch[self.batch_idxes, ESTI_START, :] + mean_trans_time + mean_proc_time           
        self.sched_opes_batch[self.batch_idxes, ope, 4] = veh.float()                    
        self.sched_opes_batch[self.batch_idxes, ope, 5] = self.feat_opes_batch[self.batch_idxes, ESTI_START, ope] + trans_times                             
        
        self.sched_mas_batch[self.batch_idxes, ma, 0] = torch.zeros(self.batch_idxes.size(0))        
        ...
        self.feat_vehs_batch[self.batch_idxes, 0, :] = torch.count_nonzero(self.ope_veh_adj_batch[self.batch_idxes, :, :], dim=1).float()                                                   
        self.feat_vehs_batch[self.batch_idxes, 1, veh] = self.time[self.batch_idxes] + trans_times                 
        utilize = self.sched_vehs_batch[self.batch_idxes, :, 2]                
        cur_time = self.time[self.batch_idxes, None].expand_as(utilize)
        utilize = torch.minimum(utilize, cur_time)
        utilize = utilize.div(self.time[self.batch_idxes, None] + 1e-9)
        ...
        if step_reward is not None:
            if step_reward == 'version1':
                max = torch.max(self.feat_opes_batch[self.batch_idxes, JOB_COMP_TIME, :], dim=1)[0]                              
    ...
    def get_makespan(self):
        ...
        mask_non_finish = (self.N+1) <= self.nums_opes                                                                 
    ...
    def get_step_reward(self, ope, proc_times):
        ...
        min_esti_start_sched = self.min_esti_start[self.batch_idxes, :] * is_scheduled                 
        ...
        self.min_esti_end = min_esti_start + self.min_proc_times_batch               
        
        min_esti_end_of_job = self.min_esti_end.gather(1, self.end_ope_biases_batch)                 
        esti_makespan = torch.max(min_esti_end_of_job, dim=1)[0]           
    ...
    def if_no_eligible(self):
        ...
        ma_eligible = ~self.mask_ma_procing_batch[:, None, :].expand(-1, self.num_jobs, -1)                           
        job_eligible = ~(self.mask_job_procing_batch + self.mask_job_finish_batch)[..., None].expand(-1, -1, self.num_mas)
        ope_ma_adj = self.ope_ma_adj_batch.gather(1, ope_step_batch[..., None].expand(-1, -1, self.num_mas)).bool()                       
    ...
    def get_dyn_adj_mat(self):
        ...
        ope_eligible = ~self.ope_status[..., None].expand(-1, -1, self.num_mas)                           
        ma_eligible = ~self.mask_ma_procing_batch[:, None, :].expand(-1, self.num_opes, -1)                           
        ope_ma_adj = self.ope_ma_adj_batch.bool()                       
        dyn_ope_ma_adj = ope_eligible & ma_eligible & ope_ma_adj                       
                                            
        ope_eligible = ~self.ope_status[..., None].expand(-1, -1, self.num_vehs)                            
        veh_eligible = ~self.mask_veh_procing_batch[:, None, :].expand(-1, self.num_opes, -1)                        
        ope_veh_adj = self.ope_veh_adj_batch.bool()                      
        dyn_ope_veh_adj = ope_eligible & veh_eligible & ope_veh_adj                        
    ...
    def next_time(self, flag_trans_2_next_time):
        ...
        avail_time = self.sched_mas_batch[:, :, 1]               
                                                                              
        job_comp_time_max = torch.max(self.feat_opes_batch[:, JOB_COMP_TIME, :])                                               
        remain_avail_time = torch.where(avail_time > self.time[:, None], avail_time, job_comp_time_max + 1.0)                                                                
                                                                                     
        transit_time = torch.min(remain_avail_time, dim=1)[0]         
                                                                    
        comp_mas = torch.where((avail_time == transit_time[:, None]) & (self.sched_mas_batch[:, :, 0] == 0) & flag_need_trans[:, None], True, False)              
        ...
        sched_mas_batch_trans = self.sched_mas_batch.transpose(1, 2)                                
        sched_mas_batch_trans[comp_mas, 0] = 1                                                             
        sched_mas_batch_trans[comp_mas, 4] = -1                             
        self.sched_mas_batch = sched_mas_batch_trans.transpose(1, 2)                             
        
        utilize = self.sched_mas_batch[:, :, 2]             
        cur_time = self.time[:, None].expand_as(utilize)
        utilize = torch.minimum(utilize, cur_time)
        utilize = utilize.div(self.time[:, None] + 1e-5)
        self.feat_mas_batch[:, 2, :] = utilize

        jobs = torch.where(comp_mas == True, self.sched_mas_batch[:, :, 3].double(), -1.0).float()                                               
        jobs_idx = np.argwhere(jobs.cpu() >= 0).to(self.device)
        jobs_idxes = jobs[jobs_idx[0], jobs_idx[1]].long()                                                      
        batch_idxes = jobs_idx[0]                                        
        ...
        avail_time_veh = self.sched_vehs_batch[:, :, 1]              
        comp_vehs = torch.where(avail_time_veh <= self.time[:, None], True, False)               
        
                                                                                                    
        self.veh_loc_batch = torch.where(self.sched_vehs_batch[:, :, 3] == comp_vehs, self.sched_vehs_batch[:, :, 3], self.veh_loc_batch).long()                                
        veh_locs = self.sched_vehs_batch[:, :, 0]
        veh_locs[comp_vehs] = 1         
    ...
    def _check_newJobInsert(self, time):
        time_resh = time[:, None].expand(-1, self.new_job_dict['release'].size(1))               
        new_jobs = (self.new_job_dict['release'] <= time_resh) & (self.new_job_dict['release'] > 0)              
    ...
    def validate_gantt(self):
        ...
        trans_time_batch = self.trans_times_batch                      
