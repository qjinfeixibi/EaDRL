...
class TFJSP_Encoder_GTrans(nn.Module):
    ...
    def forward(self, 
        ...
        if self.encoder_version == 2:
            node_emb = torch.cat([ope_emb, ma_emb, veh_emb], dim=1)                      
...
class EncoderLayer_Base(nn.Module):
    ...
    def forward(self, 
        ...
        if self.encoder_version == 2:
            ...
            proc_offload = torch.cat([proc_time, offload_trans_time_OV.transpose(1,2)], dim=2)                             
            ope_emb_out, proc_offload_out = self.ope_encoding_block(ope_emb, ma_veh_emb, proc_offload)
            proc_time_out1 = proc_offload_out[:, :, :n_mas]                     
            offload_trans_time_OV_out1 = proc_offload_out[:, :, n_mas:]                       
                          
            ma_ope_emb = torch.cat([ma_emb, ope_emb], dim=1)                              
            onload_proc = torch.cat([onload_trans_time, proc_time.transpose(1,2)], dim=2)                            
            ...
            proc_time_out = proc_time_out1 + proc_time_out2.transpose(1,2)                     
            offload_trans_time_OV_out = offload_trans_time_OV_out1.transpose(1,2) + offload_trans_time_OV_out2                       
        elif self.encoder_version == 1:
