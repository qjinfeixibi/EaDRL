...
class TFJSPModel_matnet(nn.Module):
    ...
    def init(self, state, dataset=None, loader=None):
        ...
        self.embedded_row, self.embedded_col = self.ffsp_encoder(embed_feat_ope, embed_feat_ma, norm_proc_trans_time)                                            
    ...
    def forward(self, state, baseline=False):
        ...
        embed_feat_ope_ma_veh, embed_feat_ope, embed_feat_ma,\
            embed_feat_veh, norm_proc_trans_time = self._init_embed(state)
        embedded_row, embedded_col = self.ffsp_encoder(embed_feat_ope, embed_feat_ma, norm_proc_trans_time)                                            
        embeddings = torch.cat([embedded_row, embedded_col], dim=1)
        
        self.ffsp_decoder.set_kv(embedded_row)
                                         
                                     
        encoded_current_ma, select_ma = self._get_encoding_matnet(self.embedded_col, state)                          

                                               
                                                                  
        ninf_mask_ope_on_select_ma, ninf_mask\
            = self._get_mask(state, select_ma)                                      

        if self.job_centric:
            ninf_mask_on_select_ma =\
                ninf_mask.gather(2, select_ma[:, None, :].expand(-1, ninf_mask.size(1), -1)).squeeze(-1)                 
        else:
            ninf_mask_on_select_ma = ninf_mask_ope_on_select_ma
            

                          
        all_opes_prob = self.ffsp_decoder(encoded_current_ma, ninf_mask=ninf_mask_on_select_ma.unsqueeze(1)).squeeze(1)                          
        
        batch_idxes = state.batch_idxes
        n = all_opes_prob.size(1)
        if self.training or self.model_paras['eval_type'] == 'softmax':
            while True:                                                                      
                select_job_ope = all_opes_prob.reshape(batch_size, -1).multinomial(1).squeeze(dim=1).reshape(batch_size, 1)         
                                      
                job_ope_prob = all_opes_prob.gather(1, select_job_ope)         
        ...
        if self.job_centric:
            ...
            select_job = self.from_ope_to_job(select_ope.squeeze(1), state).unsqueeze(1)            
    ...
    def _init_embed(self, state, job_centric=True):
        ...
        embed_feat_proc = self.init_embed_proc(norm_proc_trans_time[..., None])                               
    ...
    def _get_mask(self, state, select_ma):
        ...
        mask_job_on_select_ma = mask.gather(2, select_ma[:, None, :].expand(-1, self.num_jobs, -1)).squeeze(-1)              
        ...
        for batch in batch_idxes:
            select_opes_on_select_ma = state.ope_step_batch[batch, mask_job_on_select_ma[batch]]                                                            
