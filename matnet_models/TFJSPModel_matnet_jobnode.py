...
class TFJSPModel_matnet_jobnode(nn.Module):
    ...
    def forward(self, state, baseline=False):
        ...
        embedded_row, embedded_col = self.ffsp_encoder(embed_feat_job, embed_feat_ma, norm_proc_trans_time)                                            
        ...
        encoded_current_ma, select_ma = self._get_encoding_matnet(embedded_col, state)                          

                                               
                                                                  
        ninf_mask = self._get_mask(state, select_ma)                                      
        ninf_mask_job = ninf_mask.gather(2, select_ma[:, None, :].expand(-1, ninf_mask.size(1), -1)).squeeze(2)                 
                          
        all_jobs_prob = self.ffsp_decoder(encoded_current_ma, ninf_mask=ninf_mask_job.unsqueeze(1)).squeeze(1)                 
        ...
        if self.training or self.model_paras['eval_type'] == 'softmax':
            ...
            job_prob = torch.zeros(size=(batch_size, n))                      
    ...
    def _get_encoding_matnet(self, encoded_nodes, state):
        ...
        _, ma_elig_on_ope = self._get_mask_ope_ma(state)                        
    ...
    def _init_embed(self, state):
        ...
        proc_trans_time = proc_time_on_job                       
        ...
        embed_feat_job = self.init_embed_opes(norm_jobs)                        
        ...
        embed_feat_job_ma_veh = torch.cat([embed_feat_job, embed_feat_ma, embed_feat_veh], dim=1)                                        
    ...
    def _get_mask_ope_ma(self, state):
        ...
        mask_job = torch.where(mask.sum(dim=-1) > torch.zeros(size=(self.batch_size, self.num_jobs)), True, False)               
