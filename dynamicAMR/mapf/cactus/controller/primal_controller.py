...
class PRIMALController(Controller):
    ...
    def joint_policy(self, joint_observation):
        """
        推理时：joint_observation 形状 (nr_agents, C*K*K) 或 (1, nr_agents, C*K*K)
        hidden_states 形状 (1, nr_agents, 1024)
        """
        device = next(self.agent_network.parameters()).device
        joint_observation = joint_observation.view(1, self.nr_agents, -1).to(device)
        self.hidden_states = self.hidden_states.view(1, self.nr_agents, -1).to(device)
    ...
    def train(self):
        ...
        h = self.agent_network.init_hidden(batch_size).to(device)

               
        for o in obs:
                              
            o = o.view(batch_size, self.nr_agents, -1).to(device)
            h = h.view(batch_size, self.nr_agents, -1).to(device)

                
            p, v, h = self.agent_network(o, h)
                                             
            h = h.view(batch_size, self.nr_agents, -1)

                
            probs.append(p)                                                       
            values.append(v.view(batch_size, self.nr_agents))                       
        ...
        if self.vdn_mode:
            ...
            value_loss = (masked_value_error * masked_value_error).sum() / masked_total_all.clamp_min(1)
        else:
            masked_value_error = (values.view(-1) - normalized_returns.view(-1)) * mask            
            value_loss = (masked_value_error * masked_value_error).sum() / masked_total.clamp_min(1)

              
        masked_policy_loss = self.policy_loss(advantages, probs, actions.view(-1)) * mask
        policy_loss = masked_policy_loss.sum() / masked_total.clamp_min(1)
