...
def find_first_available_place(start_ctr, duration, machine_jobs):
    ...
    if machine_jobs:
        for job in machine_jobs:
            max_duration_list.append(job[3] + job[1] + job[4])                                     
...
def get_transTime(pb_instance, os, ms, trans_mat, print_=False):
    ...
    for job in os:
        ...
        tmp_trans_mat = trans_mat.gather(0, veh_loc[:, None].expand(-1, trans_mat.size(1)))                                  
        offload_trans_s = tmp_trans_mat[:, prev_ope_loc]                             
        offload_trans, veh_idx = offload_trans_s.min(dim=0)               

        onload_trans = trans_mat[prev_ope_loc, machine-1]           
...
def decode(pb_instance, os, ms, print_=False):
    ...
    trans_mat = pb_instance['trans_mat']                          
