...
class ReplayBuffer():
    ...
    def add_state_info(self, state):
        ...
        '''
        raw_opes = state.feat_opes_batch.detach().cpu().numpy()                            
        raw_mas = state.feat_mas_batch.detach().cpu().numpy()                            
        raw_vehs = state.feat_vehs_batch.detach().cpu().numpy()                            
        proc_time = state.proc_times_batch.detach().cpu().numpy()                       
        trans_time = state.trans_times_batch.detach().cpu().numpy()                    
        ope_ma_adj = state.ope_ma_adj_batch.detach().cpu().numpy()                      
        ope_pre_adj = state.ope_pre_adj_batch.detach().cpu().numpy()
        ope_sub_adj = state.ope_sub_adj_batch.detach().cpu().numpy()
        batch_idxes = state.batch_idxes.detach().cpu().numpy()        
        ope_step_batch = torch.where(state.ope_step_batch > state.end_ope_biases_batch, 
                                     state.end_ope_biases_batch, state.ope_step_batch).detach().cpu().numpy()               
...
class ReplayBuffer_ppo(ReplayBuffer):
    ...
    def add_state_info(self, state):
        ...
        raw_vehs = state.feat_vehs_batch.detach().cpu().numpy()                            
        proc_time = state.proc_times_batch.detach().cpu().numpy()                       
        trans_time = state.trans_times_batch                    
        ope_ma_adj = state.ope_ma_adj_batch.detach().cpu().numpy()                      
        ope_pre_adj = state.ope_pre_adj_batch.detach().cpu().numpy()
        ope_sub_adj = state.ope_sub_adj_batch.detach().cpu().numpy()
        batch_idxes = state.batch_idxes.detach().cpu().numpy()        
